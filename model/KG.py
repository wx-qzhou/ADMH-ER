...
class KG:
    ...
    def generate_walk(self, rt_dict, hr_dict, max_len=600):
        """
        ...
        e_max_num = min(max(e_len_num), max_len)
        r_max_num = min(max(r_len_num), max_len)
    ...
    def read_attribues(self, att_f1):
        ...
        for line in file.readlines():
            ...
            if '#' in v:
                v = v.strip('#')
            if "分钟" in v or "天" in v or "万" in v or "亿" in v:
                v = v.strip('分钟').strip('万').strip('亿').strip('天').strip()
            if "—" in v or len(v) == 0:
                continue
